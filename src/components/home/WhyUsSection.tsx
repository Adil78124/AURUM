...
function WhyUsArrowsDesktop() {
  ...
  return (
    ...
    >
      ...
      {/* sketch double-stroke curves toward center */}
      ...
      <g className="why-us-arrow-drift opacity-[0.9]" filter="url(#why-us-arrow-glow)" style={{ animationDelay: "-2.5s" }}>
        <path
...
function CenterTitle({ className = "" }: { className?: string }) {
  return (
    <div className={`relative mx-auto w-max ${className}`}>
      <div
  ...
  return (
    <section className="relative overflow-hidden bg-[#060606] py-section-gap">
      ...
      <div className="relative z-10 mx-auto max-w-[min(1380px,calc(100vw-1.5rem))] px-margin-mobile md:px-8 lg:px-10">
        {/* ——— Mobile / tablet: stack + energy ——— */}
        <div className="lg:hidden">
          <div className="relative py-4">
            <CenterTitle />
          </div>
          <MobileArrowDivider />
          <div className="mx-auto mt-6 max-w-lg space-y-6 pb-2 sm:max-w-xl">
            {CARDS.map(({ title, text, Icon, key }, i) => (
              <div key={key}>
                <div className="group border-l-2 border-[#ea580c]/70 pl-4 transition-colors hover:border-primary hover:pl-5 sm:pl-5">
        ...
        <div className="relative mx-auto hidden min-h-[min(720px,82vh)] w-full lg:block xl:min-h-[760px]">
          ...
          {CARDS.map(({ title, text, Icon, desktop, key }) => (
            ...
            >
              <div className="transition duration-500 ease-out will-change-transform group-hover:-translate-y-1">
                <div
